...
function getImageGeneratorConfig() {
  return {
    inputPath: path.resolve(process.env.PWD, mappingArg['i'] || mappingArg['input'] || generatorConfig.inputPath || './public'),
    outputPath: path.resolve(process.env.PWD, mappingArg['o'] || mappingArg['output'] || generatorConfig.outputPath || './src/generated/path/images.ts'),
    basePath: generatorConfig.basePath || '',
    includingPattern: generatorConfig.includingPattern || ['*.jpg', '*.png', '*.svg', '*.jpeg'],
