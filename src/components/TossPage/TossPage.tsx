...
function TossPage({ ...basisProps }: TossPageProps) {
  ...
  const test = async () => {
    ...
    tossPayments.requestPayment(type.value, {
      ...
      customerName: '박토스',
      successUrl: `${window.location.origin}${ROUTES.STARTER_DOCS.EXAMPLES.TOSS_SUCCESS}`,
      failUrl: `${window.location.origin}${ROUTES.STARTER_DOCS.EXAMPLES.TOSS_FAIL}`,
    });
  };
  return (
